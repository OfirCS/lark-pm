...
export default function InsightsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">
            One-click: Search all sources, extract insights, cluster, and create tickets
          </p>
        </div>
        <div className="flex items-center gap-3">
          {result && !isRunning && (
            <span className="text-sm text-stone-500">{result.stats.totalFound} items found</span>
          )}
          <button
            onClick={runMagicPipeline}
            disabled={isRunning || !company.productName}
            className="flex items-center gap-2 px-5 py-2.5 bg-stone-900 text-white rounded-xl font-medium hover:bg-stone-800 disabled:opacity-50 transition-all shadow-sm"
          >
            {isRunning ? (
      ...
      {/* Tickets Created Banner */}
      {ticketsCreated > 0 && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3">
          <CheckCircle2 size={20} className="text-emerald-600" />
          <div>
            <p className="font-medium text-emerald-900">{ticketsCreated} tickets added to review queue</p>
            <p className="text-sm text-emerald-700">
              <Link href="/dashboard/review" className="underline hover:no-underline">Go to Review Queue</Link> to review and approve them
            </p>
          </div>
        </div>
      )}
      ...
      {result && result.clusters.length === 0 && !isRunning && (
        <div className="flex flex-col items-center justify-center py-20 bg-white border border-stone-200 rounded-2xl">
          ...
          <p className="mt-2 text-stone-500 text-center max-w-md">
            ...
            Try again later or adjust your search terms.
          </p>
          <Link
            href="/onboarding"
